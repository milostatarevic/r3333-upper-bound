...
uses Lean 4.32.1, a shared Mathlib installation and the established
classification library. Its receipts do not pin every loaded Mathlib artifact;
[VERIFICATION.md](VERIFICATION.md) distinguishes this provenance from the locked
portable reproduction environment. The classification library's separate
recorded rebuild cost is 827.71 CPU-seconds.
...
The measured source/data/check payload occupied **82.92 MB**, excluding
private build directories and caches. Compiled
...
The measurements are supported by [the cost snapshot](checks/final_acceptance/COMPLETION_COST_SNAPSHOT_FINAL_001.json)
and [the nonoverlapping adjustment](checks/final_acceptance/COMPLETION_COST_FINAL_ADJUSTMENT_001.json). The adjustment adds two
