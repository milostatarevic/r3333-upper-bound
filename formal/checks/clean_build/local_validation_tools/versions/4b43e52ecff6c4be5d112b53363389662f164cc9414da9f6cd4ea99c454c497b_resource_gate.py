...
def pressure():
    ...
    def pages(label):
        return int(re.search(r'^'+re.escape(label)+r':\s*(\d+)\.',vm,re.M).group(1))
    # Do not treat memory_pressure's percentage (which includes compression)
    # as uncommitted physical RAM. File-backed pages are the recoverable cache;
    # avoid counting speculative/purgeable pages twice when already included.
    available=page_size*(pages('Pages free')+max(pages('File-backed pages'),pages('Pages speculative'),pages('Pages purgeable')))
    return level,min(ram,available),ram
def acquire(run,expected):
    start=time.monotonic()
    while True:
        with (CONTROL/'admission.lock').open('a+') as serial:
            fcntl.flock(serial,fcntl.LOCK_EX)
            level,physical_available,ram=pressure(); active=[]; available=[]
            for i in range(8):
            ...
            admit=level==1 and physical_available-reserved>=expected and bool(available)
