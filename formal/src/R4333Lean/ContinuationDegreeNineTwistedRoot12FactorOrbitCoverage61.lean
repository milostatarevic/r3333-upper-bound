import R4333Lean.ContinuationDegreeNineTwistedRoot12PhysicalSymmetry61

/-! Continuation-authored (locks/duuu), 2026-08-05.
Twisted case (slot 3, root 12) factor-orbit-coverage clone of
...
catalogues, normalizer words, and the 72 diagonal representatives are
regenerated from twisted_root12_orbits.json data (label-side k15Template
sites flipped false -> true; physical colorings renamed to the
twistedRoot12 names).  Root 12 shares the order-2 root stabilizer with
strongCase, so the 132 -> 72 halving structure carries over verbatim. -/

/-!
# Exact factor-catalogue orbit coverage in the twisted root-12 case
...
rooted normal-template involution then reduces their product from `132` to
`72` diagonal representatives.
...
namespace R4333
namespace ContinuationDegreeNineTwistedRoot12FactorOrbitCoverage61

open ProfileDHighOwnFiveRowSplit61
open ProfileDHighOwnFiveSupportC5CNF
open ContinuationDegreeNineTwistedRoot12Case61
open ContinuationDegreeNineTwistedRoot12CaseCNF61
open ContinuationDegreeNineTwistedRoot12CaseFactorCNF61
open ContinuationDegreeNineTwistedRoot12RootStabilizer61
open ContinuationDegreeNineTwistedRoot12PhysicalSymmetry61
...
def largeCataloguePermIndex : Fin 110 → Fin 120 :=
  ![9, 23, 34, 38, 53, 56, 75, 91, 109, 118,
    ...
    5, 8, 28, 36, 57, 71, 73, 93, 102, 113,
    0, 16, 25, 45, 63, 66, 78, 89, 98, 107,
    5, 8, 28, 36, 57, 71, 73, 93, 102, 113,
    4, 12, 29, 32, 51, 67, 85, 94, 106, 110,
    5, 8, 28, 36, 57, 71, 73, 93, 102, 113,
    10, 14, 33, 47, 50, 59, 87, 90, 100, 108,
    5, 8, 28, 36, 57, 71, 73, 93, 102, 113,
    2, 11, 27, 43, 58, 62, 79, 92, 96, 112]

def smallCataloguePermIndex : Fin 120 → Fin 24 :=
  ![3, 19, 9, 23, 9, 23, 3, 19, 5, 8, 2, 11,
    7, 20, 6, 17, 7, 20, 0, 16, 6, 17, 13, 22,
    13, 22, 4, 12, 15, 18, 7, 20, 1, 21, 7, 20,
    0, 16, 6, 17, 6, 17, 0, 16, 15, 18, 15, 18,
    7, 20, 1, 21, 3, 19, 0, 16, 2, 11, 13, 22,
    1, 21, 7, 20, 0, 16, 2, 11, 15, 18, 7, 20,
    1, 21, 6, 17, 0, 16, 9, 23, 3, 19, 3, 19,
    2, 11, 15, 18, 7, 20, 7, 20, 1, 21, 0, 16,
    2, 11, 7, 20, 15, 18, 6, 17, 0, 16, 1, 21,
    0, 16, 5, 8, 2, 11, 4, 12, 2, 11, 3, 19]
...
theorem exists_largeCatalogueIndex (choice : LargeFactorChoice) :
  ...
  have hcompatible : ExplicitLargeCompatible support perm := by
    ...
    change k15Template true (explicitLargeLabelOrder left)
        (explicitLargeLabelOrder right) =
      twistedRoot12LargePhysicalColoring (choice.physical left)
...
theorem exists_smallCatalogueIndex (choice : SmallFactorChoice) :
  ...
  have hcompatible : ExplicitSmallCompatible support perm := by
    ...
    change k15Template true (explicitSmallLabelOrder left)
        (explicitSmallLabelOrder right) =
      twistedRoot12SmallPhysicalColoring (choice.physical left)
        (choice.physical right) at h
    rw [hphysical left, hphysical right] at h
    exact h
  obtain ⟨index, hindexSupport, hindexPerm⟩ :=
...
def largeNormalizerWord : Fin 110 → Nat :=
  ![0, 88, 1, 2, 3, 4, 90, 89, 91, 92,
    5, 6, 7, 8, 61, 60, 62, 63, 9, 64,
    93, 94, 38, 39, 97, 42, 95, 96, 41, 40,
    85, 84, 86, 75, 72, 71, 74, 73, 82, 83,
    10, 87, 32, 54, 76, 21, 98, 109, 65, 43,
    79, 78, 46, 45, 80, 81, 47, 48, 77, 44,
    15, 14, 11, 99, 103, 102, 12, 13, 100, 101,
    66, 55, 70, 69, 59, 58, 67, 68, 56, 57,
    53, 20, 17, 16, 51, 52, 19, 18, 49, 50,
    25, 26, 23, 24, 22, 33, 35, 34, 37, 36,
    105, 104, 29, 30, 27, 28, 106, 107, 31, 108]

def smallNormalizerWord : Fin 120 → Nat :=
  ![0, 1, 2, 3, 4, 64, 5, 6, 96, 97, 7, 8,
    9, 69, 99, 98, 65, 66, 101, 102, 41, 42, 100, 40,
    45, 105, 103, 104, 79, 80, 90, 89, 92, 91, 10, 106,
    88, 76, 67, 68, 11, 107, 35, 47, 95, 59, 94, 58,
    85, 84, 49, 48, 119, 23, 86, 87, 78, 77, 15, 14,
    111, 110, 12, 13, 21, 57, 22, 118, 75, 74, 63, 62,
    72, 73, 81, 93, 60, 61, 19, 20, 54, 53, 33, 117,
    18, 17, 24, 25, 26, 27, 56, 55, 34, 46, 39, 38,
    82, 70, 28, 112, 29, 30, 52, 16, 115, 116, 113, 114,
    108, 109, 32, 31, 37, 36, 71, 83, 50, 51, 43, 44]
...
theorem freeRepresentative_compatible :
    FreePermutationCompatible (Equiv.refl (Fin 5)) := by
  intro left right hne
...
def freePhysicalRepresentative : FreeFactorChoice :=
  ⟨Equiv.refl (Fin 5), freeRepresentative_compatible⟩
...
/-! ## Rooted normal-template involution on the compact `11 × 12` product -/

def rootLargeNormalizerWord : Fin 11 → Nat :=
  ![1, 0, 91, 90, 92, 7, 64, 5, 63, 61, 65]

def rootSmallNormalizerWord : Fin 12 → Nat :=
  ![1, 0, 3, 2, 64, 6, 5, 8, 7, 69, 106, 107]

def rootLargeRepresentative (source : Fin 11) : Fin 11 :=
  Fin.ofNat 11 (rootLargeNormalizerWord source % 11)

def rootLargePhysicalAction (source : Fin 11) : Fin 10 :=
  Fin.ofNat 10 (rootLargeNormalizerWord source / 11)

def rootSmallRepresentative (source : Fin 12) : Fin 12 :=
  Fin.ofNat 12 (rootSmallNormalizerWord source % 12)

def rootSmallPhysicalAction (source : Fin 12) : Fin 10 :=
  Fin.ofNat 10 (rootSmallNormalizerWord source / 12)

theorem rootLargeNormalizerEquationExplicit :
    ∀ source : Fin 11, ∀ position : Fin 5,
      largeCataloguePhysical
          (largeRepresentativeRawIndex (rootLargeRepresentative source))
          position =
        physicalK15Map (rootLargePhysicalAction source)
          (largeCataloguePhysical (largeRepresentativeRawIndex source)
            (rootLargeAction position)) := by
  decide

theorem rootSmallNormalizerEquationExplicit :
    ∀ source : Fin 12, ∀ position : Fin 4,
      smallCataloguePhysical
          (smallRepresentativeRawIndex (rootSmallRepresentative source))
          position =
        physicalK15Map (rootSmallPhysicalAction source)
          (smallCataloguePhysical (smallRepresentativeRawIndex source)
            (rootSmallAction position)) := by
  decide

theorem rootLargeNormalizerEquation
    (source : Fin 11) (position : Fin 5) :
    (largePhysicalRepresentative
        (rootLargeRepresentative source)).physical position =
      physicalK15Equiv (rootLargePhysicalAction source)
        ((rootFlipLargeChoice
          (largePhysicalRepresentative source)).physical position) := by
  rw [rootFlipLargeChoice_physical, largePhysicalRepresentative,
    largeCatalogueChoice_physical]
  unfold largePhysicalRepresentative
  rw [largeCatalogueChoice_physical]
  exact rootLargeNormalizerEquationExplicit source position

theorem rootSmallNormalizerEquation
    (source : Fin 12) (position : Fin 4) :
    (smallPhysicalRepresentative
        (rootSmallRepresentative source)).physical position =
      physicalK15Equiv (rootSmallPhysicalAction source)
        ((rootFlipSmallChoice
          (smallPhysicalRepresentative source)).physical position) := by
  rw [rootFlipSmallChoice_physical, smallPhysicalRepresentative,
    smallCatalogueChoice_physical]
  unfold smallPhysicalRepresentative
  rw [smallCatalogueChoice_physical]
  exact rootSmallNormalizerEquationExplicit source position

noncomputable def rootPhysicalNormalizationWitness
    (largeIndex : Fin 11) (smallIndex : Fin 12) :
    PhysicalNormalizationWitness
      (rootFlipLargeChoice (largePhysicalRepresentative largeIndex))
      (largePhysicalRepresentative (rootLargeRepresentative largeIndex))
      (rootFlipSmallChoice (smallPhysicalRepresentative smallIndex))
      (smallPhysicalRepresentative (rootSmallRepresentative smallIndex)) :=
  PhysicalNormalizationWitness.ofPhysicalEquations
    (rootFlipLargeChoice (largePhysicalRepresentative largeIndex))
    (largePhysicalRepresentative (rootLargeRepresentative largeIndex))
    (rootFlipSmallChoice (smallPhysicalRepresentative smallIndex))
    (smallPhysicalRepresentative (rootSmallRepresentative smallIndex))
    (rootLargePhysicalAction largeIndex)
    (rootSmallPhysicalAction smallIndex)
    (rootLargeNormalizerEquation largeIndex)
    (rootSmallNormalizerEquation smallIndex)

/-! ## The exact 72 representative pairs and coverage -/

def quotientRepresentativePair : Fin 72 → Fin 11 × Fin 12 :=
    ...
    ∀ largeIndex : Fin 11, ∀ smallIndex : Fin 12,
      ∃ representative : Fin 72,
        (largeIndex = (quotientRepresentativePair representative).1 ∧
          smallIndex = (quotientRepresentativePair representative).2) ∨
        (rootLargeRepresentative largeIndex =
            (quotientRepresentativePair representative).1 ∧
          rootSmallRepresentative smallIndex =
            (quotientRepresentativePair representative).2) := by
  decide

theorem factorAssignmentRealization_reduces_to_72
    ...
    ∃ representative : Fin 72,
      ∃ normalized : CanonicalBranch .uuu 2 (Fin.last 15),
  ...
  obtain ⟨representative, hdirect | hflipped⟩ :=
    quotientRepresentativePair_coverage largeIndex smallIndex
  · rcases hdirect with ⟨hlarge, hsmall⟩
    subst hlarge
    subst hsmall
    exact ⟨representative, firstBranch, firstRealization⟩
  · rcases hflipped with ⟨hlarge, hsmall⟩
    let flipped := firstRealization.rootFlip
    let rootWitness := rootPhysicalNormalizationWitness largeIndex smallIndex
    let secondBranch := normalizedBranch firstBranch
      (rootFlipFreeChoice freePhysicalRepresentative)
      freePhysicalRepresentative rootWitness
    have secondRealization : FactorAssignmentRealization secondBranch
        freePhysicalRepresentative
        (largePhysicalRepresentative (rootLargeRepresentative largeIndex))
        (smallPhysicalRepresentative (rootSmallRepresentative smallIndex)) :=
      flipped.normalize rootWitness
    rw [hlarge, hsmall] at secondRealization
    exact ⟨representative, secondBranch, secondRealization⟩

theorem twistedRoot12Realization_reduces_to_72
  ...
  exact factorAssignmentRealization_reduces_to_72
...
#print axioms rootLargeNormalizerEquationExplicit
#print axioms rootSmallNormalizerEquationExplicit
#print axioms rootPhysicalNormalizationWitness
#print axioms quotientRepresentativePair_coverage
#print axioms factorAssignmentRealization_reduces_to_72
#print axioms twistedRoot12Realization_reduces_to_72

end ContinuationDegreeNineTwistedRoot12FactorOrbitCoverage61
end R4333
