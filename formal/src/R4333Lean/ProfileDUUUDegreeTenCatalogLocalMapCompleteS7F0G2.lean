...
theorem compatibleMap_complete_support7_false_group2
    (perm : Equiv.Perm (Fin 5))
    (hmap : IsCompatiblePerm .support7 false 2 perm) :
    ∃ action : Fin 10,
      ∀ position, perm position =
        compatibleMap .support7 false 2 action position := by
  fin_cases perm <;> (revert hmap; unfold IsCompatiblePerm; decide)

#print axioms compatibleMap_complete_support7_false_group2
