...
theorem compatibleMap_complete_support2_false_group0
    (perm : Equiv.Perm (Fin 5))
    (hmap : IsCompatiblePerm .support2 false 0 perm) :
    ∃ action : Fin 10,
      ∀ position, perm position =
        compatibleMap .support2 false 0 action position := by
  fin_cases perm <;> (revert hmap; unfold IsCompatiblePerm; decide)

#print axioms compatibleMap_complete_support2_false_group0
