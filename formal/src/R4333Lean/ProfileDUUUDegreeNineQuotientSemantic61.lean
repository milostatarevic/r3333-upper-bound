...
import R4333Lean.ProfileDUUUDegreeNineQuotientFormula61

/-!
# Semantic soundness of the exact 72-representative degree-nine quotient

The companion formula module fixes the clause/index data.  This module proves
that every `StrongRealization` produces a satisfying valuation of that exact
formula.  Consequently a kernel-checked LRAT refutation of the single quotient
formula rules out the strongest degree-nine case.
-/

namespace R4333
namespace ProfileDUUUDegreeNineQuotientSemantic61
...
private theorem quotient_satisfies_guarded_support_selected
  ...
  · apply satisfies_of_mem_holds
    ...
    · simpa [ProfileDUUUDegreeNineStrongCaseCNF61.supportLiteral,
        hmember] using
...
explicit leaves fills the existing strongest degree-nine obstruction slot. -/
theorem strongCaseObstruction_of_quotient_leaf_lrats
    (hunsat : ∀ representative : Fin 72,
      QuotientLeafLRATRefutation representative) :
    DegreeNineCaseObstruction strongCase := by
  intro realization
...
theorem strongRealization_yields_satisfying_quotientFmla
...
theorem noStrongRealization_of_quotient_lrat
  ...
  obtain ⟨valuation, hsatisfies⟩ :=
    strongRealization_yields_satisfying_quotientFmla realization
  exact hunsat valuation hsatisfies

/-- The quotient certificate fills the existing semantic obstruction slot for
the strongest residual degree-nine label. -/
theorem strongCaseObstruction_of_quotient_lrat
    (hunsat : QuotientLRATRefutation) :
    DegreeNineCaseObstruction strongCase := by
  intro realization
...
#print axioms strongCaseObstruction_of_quotient_leaf_lrats
#print axioms strongRealization_yields_satisfying_quotientFmla
#print axioms noStrongRealization_of_quotient_lrat
#print axioms strongCaseObstruction_of_quotient_lrat

end ProfileDUUUDegreeNineQuotientSemantic61
end R4333
