...
theorem compatibleMap_complete_support7_true_group1
    (perm : Equiv.Perm (Fin 5))
    (hmap : IsCompatiblePerm .support7 true 1 perm) :
    ∃ action : Fin 10,
      ∀ position, perm position =
        compatibleMap .support7 true 1 action position := by
  fin_cases perm <;> (revert hmap; unfold IsCompatiblePerm; decide)

#print axioms compatibleMap_complete_support7_true_group1
