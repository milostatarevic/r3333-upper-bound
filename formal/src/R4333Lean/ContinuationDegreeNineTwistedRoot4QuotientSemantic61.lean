...
import R4333Lean.ContinuationDegreeNineTwistedRoot4QuotientFormula61

/-! Continuation-authored (locks/duuu), 2026-08-05.
Twisted case (slot 3, root 4) quotient-semantic clone of
ProfileDUUUDegreeNineQuotientSemantic61.  This is the per-case endpoint: it
ties the root-4 quotient formula back to
`DegreeNineCaseObstruction twistedRoot4Case`.  The proof text is verbatim
from the archive original up to renames: the strongCase companion namespaces
become their root-4 clones, `StrongRealization` becomes
`TwistedRoot4Realization`, `strongCase` becomes `twistedRoot4Case`,
`strongRealization_reduces_to_72` becomes
`twistedRoot4Realization_reduces_to_132` (the root-4 label stabilizer is
trivial, so the selector count is the full `11 x 12 = 132` product and
`Fin 72` becomes `Fin 132` throughout), and
`strongFmla_outside_selectorNamespace` becomes
`twistedRoot4Fmla_outside_selectorNamespace`.  There is no RootStabilizer
module for root 4, so that `open` is dropped.  One proof differs from the
verbatim clone: `quotientRepresentativeFree_label` must route through the
free representative involution `Equiv.swap 1 2` (root 4's identity free
permutation is template-incompatible; see the matching fix in the companion
formula module's `quotientFreeLabel`).  No k15Template occurs here and no
data tables live here; the module is LRAT-trace-free by construction
(imports only ClauseCore and the root-4 quotient formula module). -/

/-!
# Semantic soundness of the exact 132-representative root-4 quotient

The companion formula module fixes the clause/index data.  This module proves
that every `TwistedRoot4Realization` produces a satisfying valuation of that
exact formula.  Consequently a kernel-checked LRAT refutation of the single
quotient formula rules out the twisted root-4 degree-nine case.
-/

namespace R4333
namespace ContinuationDegreeNineTwistedRoot4QuotientSemantic61
...
open ContinuationDegreeNineTwistedRoot4CaseCNF61
open ContinuationDegreeNineTwistedRoot4CaseFactorCNF61
open ContinuationDegreeNineTwistedRoot4PhysicalSymmetry61
...
private theorem quotient_satisfies_guarded_support_selected
    ...
    (quotientFinalValuation selected branch).satisfies
      [Sat.Literal.neg selector,
       ContinuationDegreeNineTwistedRoot4CaseCNF61.supportLiteral
        (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
  ...
  · apply satisfies_of_mem_holds
    ...
    · simpa [ContinuationDegreeNineTwistedRoot4CaseCNF61.supportLiteral,
        hmember] using
...
/-- Unlike strongCase/root 12, the root-4 free representative permutation is
the involution `Equiv.swap 1 2` (the identity is incompatible with the
twisted root-4 template), so both sides carry the swapped label order and
the bridge goes through `explicitFreeLabelOrder_eq` at the swapped
position. -/
theorem quotientRepresentativeFree_label
  ...
  show (freeLabelOrder (freePhysicalRepresentative.1 position)).1 =
    quotientFreeLabel representative position
  rw [← explicitFreeLabelOrder_eq]
  rfl
...
explicit leaves fills the existing twisted root-4 degree-nine obstruction
slot. -/
theorem twistedRoot4CaseObstruction_of_quotient_leaf_lrats
    (hunsat : ∀ representative : Fin 132,
      QuotientLeafLRATRefutation representative) :
    DegreeNineCaseObstruction twistedRoot4Case := by
  intro realization
...
theorem twistedRoot4Realization_yields_satisfying_quotientFmla
...
theorem noStrongRealization_of_quotient_lrat
  ...
  obtain ⟨valuation, hsatisfies⟩ :=
    twistedRoot4Realization_yields_satisfying_quotientFmla realization
  exact hunsat valuation hsatisfies

/-- The quotient certificate fills the existing semantic obstruction slot for
the twisted root-4 degree-nine label. -/
theorem twistedRoot4CaseObstruction_of_quotient_lrat
    (hunsat : QuotientLRATRefutation) :
    DegreeNineCaseObstruction twistedRoot4Case := by
  intro realization
...
#print axioms twistedRoot4CaseObstruction_of_quotient_leaf_lrats
#print axioms twistedRoot4Realization_yields_satisfying_quotientFmla
#print axioms noStrongRealization_of_quotient_lrat
#print axioms twistedRoot4CaseObstruction_of_quotient_lrat

end ContinuationDegreeNineTwistedRoot4QuotientSemantic61
end R4333
