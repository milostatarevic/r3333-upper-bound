import R4333Lean.ProfileDUUUDegreeNineStrongCaseFactorCNF61

/-!
# Rooted normal-template stabilizer in the strong UUU degree-nine case

The untwisted critical `K15` has one nonidentity colour-preserving
automorphism fixing the normalized root label `2`.  This file packages that
...
namespace R4333
namespace ProfileDUUUDegreeNineRootStabilizer61
...
theorem rootFlipMap_preserves (left right : Fin 15) (hne : left ≠ right) :
    k15Template false (rootFlipMap left) (rootFlipMap right) =
      k15Template false left right := by
  fin_cases left <;> fin_cases right <;> simp_all <;> decide

theorem rootFlipMap_root : rootFlipMap 2 = 2 := by decide

noncomputable def rootFlipEquiv : Equiv.Perm (Fin 15) :=
  Equiv.ofBijective rootFlipMap rootFlipMap_bijective

def rootFreeAction : Fin 5 → Fin 5 := ![4, 3, 2, 1, 0]
def rootLargeAction : Fin 5 → Fin 5 := ![4, 3, 2, 1, 0]
...
def explicitFreeLabelOrder : Fin 5 → Fin 15 := ![3, 8, 10, 13, 14]
def explicitLargeLabelOrder : Fin 5 → Fin 15 := ![4, 6, 7, 11, 12]
def explicitSmallLabelOrder : Fin 4 → Fin 15 := ![0, 1, 5, 9]

private theorem explicitFreeLabelOrder_mem (position : Fin 5) :
    explicitFreeLabelOrder position ∈ strongFreeLabelSet := by
  fin_cases position <;> decide

private theorem explicitLargeLabelOrder_mem (position : Fin 5) :
    explicitLargeLabelOrder position ∈ strongLargeLabelSet := by
  fin_cases position <;> decide

private theorem explicitSmallLabelOrder_mem (position : Fin 4) :
    explicitSmallLabelOrder position ∈ strongSmallLabelSet := by
  fin_cases position <;> decide
...
theorem explicitFreeLabelOrder_eq (position : Fin 5) :
  ...
  have hfun : explicitFreeLabelOrder =
      strongFreeLabelSet.orderEmbOfFin strongFreeLabelSet_card :=
    Finset.orderEmbOfFin_unique strongFreeLabelSet_card
...
theorem explicitLargeLabelOrder_eq (position : Fin 5) :
  ...
  have hfun : explicitLargeLabelOrder =
      strongLargeLabelSet.orderEmbOfFin strongLargeLabelSet_card :=
    Finset.orderEmbOfFin_unique strongLargeLabelSet_card
...
theorem explicitSmallLabelOrder_eq (position : Fin 4) :
  ...
  have hfun : explicitSmallLabelOrder =
      strongSmallLabelSet.orderEmbOfFin strongSmallLabelSet_card :=
    Finset.orderEmbOfFin_unique strongSmallLabelSet_card
...
noncomputable def rootFlipLargeChoice
  ...
  compatible := by
    ...
    change k15Template false (largeLabelOrder left).1
...
noncomputable def rootFlipSmallChoice
  ...
  compatible := by
    ...
    change k15Template false (smallLabelOrder left).1
...
theorem rootFlip_largeSmall_color
    ...
    k15Template false
      ...
      k15Template false
        ...
        (small.label (rootSmallAction smallIndex)) := by
  change k15Template false (largeLabelOrder largeIndex).1
      (smallLabelOrder smallIndex).1 =
    k15Template false
      (largeLabelOrder (rootLargeAction largeIndex)).1
      (smallLabelOrder (rootSmallAction smallIndex)).1
...
end ProfileDUUUDegreeNineRootStabilizer61
end R4333
