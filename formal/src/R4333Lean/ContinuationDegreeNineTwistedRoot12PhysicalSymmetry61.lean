import R4333Lean.ContinuationDegreeNineTwistedRoot12RootStabilizer61
import R4333Lean.ProfileDDegreeTenMatrixTransport61

/-! Continuation-authored (locks/duuu), 2026-08-05.
Twisted case (slot 3, root 12) physical-symmetry clone of
ProfileDUUUDegreeNinePhysicalSymmetry61.  Physical tables and tail/free
transports are case-independent (verbatim); the label-side k15Template
sites are flipped false -> true; unlike roots 1/4, the root-12 label
stabilizer is nontrivial, so the rootFlip section is kept, backed by
ContinuationDegreeNineTwistedRoot12RootStabilizer61. -/

/-!
# Physical symmetry transport for the twisted root-12 UUU degree-nine case
...
namespace R4333
namespace ContinuationDegreeNineTwistedRoot12PhysicalSymmetry61
...
/-! ## Diagonal normal-template root flip -/

theorem FactorAssignmentRealization.rootFlip
    {branch : CanonicalBranch .uuu 2 (Fin.last 15)}
    {free : FreeFactorChoice} {large : LargeFactorChoice}
    {small : SmallFactorChoice}
    (realization : FactorAssignmentRealization branch free large small) :
    FactorAssignmentRealization branch
      (rootFlipFreeChoice free) (rootFlipLargeChoice large)
      (rootFlipSmallChoice small) where
  largeSupport := by
    intro position
    exact realization.largeSupport position
  smallSupport := by
    intro position
    exact realization.smallSupport position
  freeLarge := by
    intro freeIndex largeIndex
    rw [rootFlipLargeChoice_physical]
    calc
      _ = decodedNormalHostColor
          (k15Template true (free.label freeIndex)
            (large.label (rootLargeAction largeIndex))) :=
        realization.freeLarge freeIndex (rootLargeAction largeIndex)
      _ = _ := congrArg decodedNormalHostColor
        (rootFlip_freeLarge_color free large freeIndex largeIndex).symm
  freeSmall := by
    intro freeIndex smallIndex
    rw [rootFlipSmallChoice_physical]
    calc
      _ = decodedNormalHostColor
          (k15Template true (free.label freeIndex)
            (small.label (rootSmallAction smallIndex))) :=
        realization.freeSmall freeIndex (rootSmallAction smallIndex)
      _ = _ := congrArg decodedNormalHostColor
        (rootFlip_freeSmall_color free small freeIndex smallIndex).symm
  largeSmall := by
    intro largeIndex smallIndex
    rw [rootFlipLargeChoice_physical, rootFlipSmallChoice_physical]
    calc
      _ = decodedNormalHostColor
          (k15Template true
            (large.label (rootLargeAction largeIndex))
            (small.label (rootSmallAction smallIndex))) :=
        realization.largeSmall (rootLargeAction largeIndex)
          (rootSmallAction smallIndex)
      _ = _ := congrArg decodedNormalHostColor
        (rootFlip_largeSmall_color large small largeIndex smallIndex).symm
...
theorem freeChoiceNormalizer_preserves_normalColor
    ...
    (left right : FiveVertex) (hne : left ≠ right) :
    twistedRoot12FreeNormalColor (freeChoiceNormalizer source target left)
...
theorem freeChoiceNormalizer_preserves_cycle
  ...
  unfold twistedRoot12FreeNormalColor at hnormal
...
#print axioms FactorAssignmentRealization.rootFlip
#print axioms FactorAssignmentRealization.normalize

end ContinuationDegreeNineTwistedRoot12PhysicalSymmetry61
end R4333
