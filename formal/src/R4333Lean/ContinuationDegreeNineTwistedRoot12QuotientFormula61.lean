import R4333Lean.ContinuationDegreeNineTwistedRoot12FactorOrbitCoverage61

/-! Continuation-authored (locks/duuu), 2026-08-05.
Twisted case (slot 3, root 12) quotient-formula clone of
ProfileDUUUDegreeNineQuotientFormula61.  Root 12 keeps the strongCase
selector count (72 representatives, `1 + 72 * 95 = 6841` suffix clauses,
matching twisted_root12_quotient.cnf).  The pre-reduced physical tables are
regenerated from the root-12 catalogue data; the three label-side
k15Template sites flip false -> true (the binders feed
explicit{Free,Large,Small}LabelOrder, i.e. label-set vertices); the
physical-side machinery is identical to strongCase. -/

/-!
# Exact 72-representative quotient formula for the twisted root-12 case

This file contains only formula and index data.  The semantic reason that the
72 representatives are complete lives in
`ContinuationDegreeNineTwistedRoot12FactorOrbitCoverage61`; satisfaction and
...
namespace R4333
namespace ContinuationDegreeNineTwistedRoot12QuotientFormula61
...
root-12 case, and each leaf appends the 95 payload units obtained by removing
the selector head from that representative's guarded clauses.
-/

/-- The authoritative base formula for the twisted root-12 case (physically
...
end ContinuationDegreeNineTwistedRoot12QuotientFormula61
end R4333
