import R4333Lean.ContinuationDegreeNineTwistedRoot4CaseFactorCNF61
import R4333Lean.ProfileDDegreeTenMatrixTransport61

/-! Continuation-authored (locks/duuu), 2026-08-05.
Twisted case (slot 3, root 4) physical-symmetry clone of
...
k15Template sites are flipped false -> true; the root-4 label stabilizer
is trivial, so the rootFlip section and the RootStabilizer import/open
are dropped. -/

/-!
# Physical symmetry transport for the twisted root-4 UUU degree-nine case
...
namespace R4333
namespace ContinuationDegreeNineTwistedRoot4PhysicalSymmetry61
...
theorem freeChoiceNormalizer_preserves_normalColor
    ...
    (left right : FiveVertex) (hne : left ≠ right) :
    twistedRoot4FreeNormalColor (freeChoiceNormalizer source target left)
...
theorem freeChoiceNormalizer_preserves_cycle
  ...
  unfold twistedRoot4FreeNormalColor at hnormal
...
end ContinuationDegreeNineTwistedRoot4PhysicalSymmetry61
end R4333
