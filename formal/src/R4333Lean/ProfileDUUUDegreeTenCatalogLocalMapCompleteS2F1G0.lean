...
theorem compatibleMap_complete_support2_true_group0
    (perm : Equiv.Perm (Fin 5))
    (hmap : IsCompatiblePerm .support2 true 0 perm) :
    ∃ action : Fin 10,
      ∀ position, perm position =
        compatibleMap .support2 true 0 action position := by
  fin_cases perm <;> (revert hmap; unfold IsCompatiblePerm; decide)

#print axioms compatibleMap_complete_support2_true_group0
