...
namespace R4333
namespace ContinuationDegreeNineTwistedRoot12QuotientSemantic61
...
private theorem quotient_satisfies_guarded_support_selected
    ...
    (quotientFinalValuation selected branch).satisfies
      [Sat.Literal.neg selector,
       ContinuationDegreeNineTwistedRoot12CaseCNF61.supportLiteral
        (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
  ...
  · apply satisfies_of_mem_holds
    ...
    · simpa [ContinuationDegreeNineTwistedRoot12CaseCNF61.supportLiteral,
        hmember] using
...
def QuotientLeafLRATRefutation (representative : Fin 72) : Prop :=
...
def quotientLeafCoreFmla (representative : Fin 72)
...
theorem satisfies_quotientLeafCoreFmla
    (representative : Fin 72) (baseGaps unitGaps : List Nat)
...
/-- Conditional semantic closure: a checked refutation for each of the 72
explicit leaves fills the existing twisted root-12 degree-nine obstruction
slot. -/
theorem twistedRoot12CaseObstruction_of_quotient_leaf_lrats
    (hunsat : ∀ representative : Fin 72,
      QuotientLeafLRATRefutation representative) :
    DegreeNineCaseObstruction twistedRoot12Case := by
  intro realization
...
theorem twistedRoot12Realization_yields_satisfying_quotientFmla
...
the explicit 72-selector quotient formula. -/
...
theorem noStrongRealization_of_quotient_lrat
  ...
  obtain ⟨valuation, hsatisfies⟩ :=
    twistedRoot12Realization_yields_satisfying_quotientFmla realization
  exact hunsat valuation hsatisfies

/-- The quotient certificate fills the existing semantic obstruction slot for
the twisted root-12 degree-nine label. -/
theorem twistedRoot12CaseObstruction_of_quotient_lrat
    (hunsat : QuotientLRATRefutation) :
    DegreeNineCaseObstruction twistedRoot12Case := by
  intro realization
...
#print axioms twistedRoot12CaseObstruction_of_quotient_leaf_lrats
#print axioms twistedRoot12Realization_yields_satisfying_quotientFmla
#print axioms noStrongRealization_of_quotient_lrat
#print axioms twistedRoot12CaseObstruction_of_quotient_lrat

end ContinuationDegreeNineTwistedRoot12QuotientSemantic61
end R4333
