import PricingG.UUTD9Root4Case
...
`UUTD9Root4Case`.  A physical support pair consists of an
...
def twistedRoot4FreeLabelSet : Finset (Fin 15) :=
  colorNeighborhood (k15Template true) 0 4

def twistedRoot4LargeLabelSet : Finset (Fin 15) :=
  colorNeighborhood (k15Template true) 2 4

theorem twistedRoot4FreeLabelSet_card : twistedRoot4FreeLabelSet.card = 5 := by decide

theorem twistedRoot4LargeLabelSet_card : twistedRoot4LargeLabelSet.card = 5 := by decide

theorem twistedRoot4SmallLabelSet_card : twistedRoot4SmallLabelSet.card = 4 := by decide

def twistedRoot4PairOfSupports?
    (large small : Finset (Fin 15)) : Option StrongPair :=
  if hlarge : large ∈ twistedRoot4LargeSupportCandidates then
    if hsmall : small ∈ twistedRoot4CompatibleSmallSupportCandidates then
...
def twistedRoot4LargePhysicalColorMap : ThreeColor → ThreeColor := ![0, 1, 0]

def twistedRoot4LargePhysicalColoring : EdgeColoring (Fin 15) ThreeColor where
  color left right := twistedRoot4LargePhysicalColorMap
    (k15Template false left right)
  color_symm left right := by
    exact congrArg twistedRoot4LargePhysicalColorMap
...
def twistedRoot4FreeNormalColor (left right : FiveVertex) : ThreeColor :=
...
def freeLabelOrder : Fin 5 ≃ ↑twistedRoot4FreeLabelSet :=
  (Finset.orderIsoOfFin twistedRoot4FreeLabelSet twistedRoot4FreeLabelSet_card).toEquiv

def largeLabelOrder : Fin 5 ≃ ↑twistedRoot4LargeLabelSet :=
  (Finset.orderIsoOfFin twistedRoot4LargeLabelSet twistedRoot4LargeLabelSet_card).toEquiv

def smallLabelOrder : Fin 4 ≃ ↑twistedRoot4SmallLabelSet :=
  (Finset.orderIsoOfFin twistedRoot4SmallLabelSet twistedRoot4SmallLabelSet_card).toEquiv
...
def FreePermutationCompatible (freePerm : Equiv.Perm (Fin 5)) : Prop :=
  ∀ left right : FiveVertex, left ≠ right →
    k15Template true
      ...
      twistedRoot4FreeNormalColor left right
...
noncomputable def freeLabelEquiv (realization : TwistedRoot4Realization) :
    FiveVertex ≃ ↑twistedRoot4FreeLabelSet :=
  embeddingEquivRange
    ...
    twistedRoot4FreeLabelSet (by
      simpa [twistedRoot4FreeLabelSet] using realization.freeLabel_image)

noncomputable def largeLabelEquiv (realization : TwistedRoot4Realization) :
    FiveVertex ≃ ↑twistedRoot4LargeLabelSet :=
  embeddingEquivRange
    (⟨largeLabel realization, by
      ...
      apply restLabel_injective_local
      exact realization.normal.vertex.symm.injective h⟩)
    twistedRoot4LargeLabelSet (by
      simpa [twistedRoot4LargeLabelSet] using realization.largeLabel_image)

theorem TwistedRoot4Realization.largeTarget_eq_one
...
theorem TwistedRoot4Realization.largeTargetSize_eq_fifteen
    (realization : TwistedRoot4Realization) :
    blockSize (largeForeignBlock realization.branch.toSemantic.block
      realization.skeleton.firstLarge) = 15 := by
  rw [TwistedRoot4Realization.largeTarget_eq_one realization]
  rfl

noncomputable def largePhysicalEmbedding
    (realization : TwistedRoot4Realization) : Fin 5 ↪ Fin 15 where
  toFun index := Fin.cast
    (TwistedRoot4Realization.largeTargetSize_eq_fifteen realization)
...
noncomputable def twistedRoot4LargePhysicalSupport (realization : TwistedRoot4Realization) :
    Finset (Fin 15) :=
  Finset.univ.image (largePhysicalEmbedding realization)

theorem TwistedRoot4Realization.largePhysicalEmbedding_mem
    (realization : TwistedRoot4Realization) (index : Fin 5) :
  ...
  simpa [largePhysicalEmbedding,
    TwistedRoot4Realization.largeTarget_eq_one realization] using hcolor

theorem TwistedRoot4Realization.largePhysicalSupport_eq
    (realization : TwistedRoot4Realization) :
  ...
  · intro position hposition
    ...
    exact TwistedRoot4Realization.largePhysicalEmbedding_mem realization index
  · rw [show (twistedRoot4LargeSupport realization).card = 5 from
      (mem_independentSupportCandidates_iff 5 2
        (twistedRoot4LargeSupport realization)).mp
          realization.largeSupport_mem_candidates |>.1]
    simp [twistedRoot4LargePhysicalSupport,
      Finset.card_image_of_injective _
        (largePhysicalEmbedding realization).injective]

theorem TwistedRoot4Realization.largePhysicalSupport_mem_candidates
    (realization : TwistedRoot4Realization) :
    twistedRoot4LargePhysicalSupport realization ∈
      twistedRoot4LargeSupportCandidates := by
  rw [TwistedRoot4Realization.largePhysicalSupport_eq realization]
  exact realization.largeSupport_mem_candidates

noncomputable def twistedRoot4SupportPair
    (realization : TwistedRoot4Realization) : StrongPair :=
  ⟨⟨twistedRoot4LargePhysicalSupport realization,
...
noncomputable def largePhysicalEquiv
    (realization : TwistedRoot4Realization) :
    Fin 5 ≃ ↑(twistedRoot4LargePhysicalSupport realization) :=
  embeddingEquivRange (largePhysicalEmbedding realization)
    (twistedRoot4LargePhysicalSupport realization) rfl

theorem TwistedRoot4Realization.free_free_edge
    (realization : TwistedRoot4Realization) (left right : FiveVertex)
    (hlr : left ≠ right) :
...
private theorem decode_blockOne_template
    ...
    decodeNonBlockColor (blockRootColor 2) (canonicalPalette 2 q) =
      twistedRoot4LargePhysicalColorMap q := by
  fin_cases q
...
theorem TwistedRoot4Realization.large_large_edge_physical
    (realization : TwistedRoot4Realization) (left right : Fin 5)
    (hlr : left ≠ right) :
  ...
  have hleftMem : largePhysicalEmbedding realization left ∈
      twistedRoot4LargeSupport realization := by
    exact TwistedRoot4Realization.largePhysicalEmbedding_mem realization left
  have hrightMem : largePhysicalEmbedding realization right ∈
      twistedRoot4LargeSupport realization := by
    exact TwistedRoot4Realization.largePhysicalEmbedding_mem realization right
  have hindependent :=
    (mem_independentSupportCandidates_iff 5 2
      (twistedRoot4LargeSupport realization)).mp
  ...
  simpa [twistedRoot4LargePhysicalColoring] using hedge

/-! The canonical row transport, retaining its permutation explicitly. -/

structure CanonicalStrongData (realization : TwistedRoot4Realization) where
...
theorem exists_canonicalStrongData (realization : TwistedRoot4Realization) :
...
noncomputable def semanticFreeEquiv
    ...
    Fin 5 ≃ ↑twistedRoot4FreeLabelSet :=
...
noncomputable def semanticLargePairEquiv
    (realization : TwistedRoot4Realization) :
    Fin 5 ≃ ↑(twistedRoot4SupportPair realization).1.1 := by
  change Fin 5 ≃ ↑(twistedRoot4LargePhysicalSupport realization)
  exact semanticLargeEquiv realization

noncomputable def semanticSmallPairEquiv
    (realization : TwistedRoot4Realization) :
    Fin 4 ≃ ↑(twistedRoot4SupportPair realization).2.1 := by
  change Fin 4 ≃ ↑(twistedRoot4SmallSupport realization)
...
noncomputable def semanticAssignmentData
  ...
  largePerm := (semanticLargePairEquiv realization).trans
    (pairLargeOrder (twistedRoot4SupportPair realization)).symm
  smallPerm := (semanticSmallPairEquiv realization).trans
    (pairSmallOrder (twistedRoot4SupportPair realization)).symm
...
theorem semanticAssignmentData_largePhysical
    ...
    (semanticAssignmentData realization data).largePhysical
        (twistedRoot4SupportPair realization) index =
      largePhysicalEmbedding realization
        ((largeLabelEquiv realization).symm (largeLabelOrder index)) := by
  change (pairLargeOrder (twistedRoot4SupportPair realization)
    (((semanticLargePairEquiv realization).trans
      (pairLargeOrder (twistedRoot4SupportPair realization)).symm) index)).1 = _
  rw [Equiv.trans_apply, Equiv.apply_symm_apply]
...
theorem semanticAssignmentData_smallPhysical
    ...
    (semanticAssignmentData realization data).smallPhysical
      ...
      smallPhysicalEmbedding realization
        ((smallLabelEquiv realization).symm (smallLabelOrder index)) := by
  change (pairSmallOrder (twistedRoot4SupportPair realization)
    (((semanticSmallPairEquiv realization).trans
      (pairSmallOrder (twistedRoot4SupportPair realization)).symm) index)).1 = _
  rw [Equiv.trans_apply, Equiv.apply_symm_apply]
...
theorem decode_canonicalInside_eq_strongFreeNormalColor
    ...
    decodeNonBlockColor 3
      ...
      twistedRoot4FreeNormalColor left right := by
  by_cases hcycle : canonicalCycleWord.coloring left right
  · simp [twistedRoot4FreeNormalColor, hcycle, decodeRemainingColor,
...
theorem semanticAssignmentData_compatible
    (realization : TwistedRoot4Realization)
    (data : CanonicalStrongData realization) :
    LocalAssignmentCompatible (twistedRoot4SupportPair realization)
  ...
  · intro left right hlr
    ...
    rw [semanticAssignmentData_freeLabel,
      ...
      TwistedRoot4Realization.free_free_edge realization (data.rowPerm left)
  ...
  · intro left right hlr
    ...
    change k15Template true
      ...
      twistedRoot4LargePhysicalColoring
        ((semanticAssignmentData realization data).largePhysical
          (twistedRoot4SupportPair realization) left)
        ((semanticAssignmentData realization data).largePhysical
          (twistedRoot4SupportPair realization) right)
    ...
    exact TwistedRoot4Realization.large_large_edge_physical realization
...
noncomputable def semanticAssignment
    (realization : TwistedRoot4Realization)
    (data : CanonicalStrongData realization) :
    LocalAssignment (twistedRoot4SupportPair realization) :=
  ⟨semanticAssignmentData realization data,
    semanticAssignmentData_compatible realization data⟩

theorem TwistedRoot4Realization.smallSupport_eq_foreignZero
    (realization : TwistedRoot4Realization) :
    twistedRoot4SmallSupport realization =
...
theorem TwistedRoot4Realization.free_large_host_edge
    (realization : TwistedRoot4Realization) (free large : Fin 5) :
    realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.rows
...
theorem TwistedRoot4Realization.free_small_host_edge
    (realization : TwistedRoot4Realization) (free : Fin 5) (small : Fin 4) :
    realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.rows
        free (blockVertex 0
...
theorem TwistedRoot4Realization.large_small_host_edge
    (realization : TwistedRoot4Realization) (large : Fin 5) (small : Fin 4) :
    realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.tail.coloring
...
theorem twistedRoot4BaseFmla_outside_selectorNamespace :
    FormulaOutsideNamespace selectorNamespaceTag
...
theorem twistedRoot4Fmla_outside_selectorNamespace :
    FormulaOutsideNamespace selectorNamespaceTag
  ...
  · exact twistedRoot4BaseFmla_outside_selectorNamespace
  · intro clause hclause literal hliteral
...
noncomputable def twistedRoot4FinalValuation
    (realization : TwistedRoot4Realization)
...
theorem twistedRoot4FinalValuation_ordinaryEdgeAtom
    (realization : TwistedRoot4Realization)
    (data : CanonicalStrongData realization)
    (left right : TotalVertex) (hne : left ≠ right) (q : Fin 4) :
    twistedRoot4FinalValuation realization data
      ...
      totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
        left right = q := by
  unfold twistedRoot4FinalValuation
...
private theorem twistedRoot4Realization_selectedBlockVertex
    (realization : TwistedRoot4Realization) :
    blockVertex realization.branch.toSemantic.block
        realization.branch.toSemantic.position.val =
      blockVertex 2 (Fin.last 15).val := by
  change blockVertex
      (uuuRepresentativeIndexBySlot (caseSlot twistedRoot4Case)).1
      (uuuRepresentativeIndexBySlot (caseSlot twistedRoot4Case)).2.val =
    blockVertex 2 (Fin.last 15).val
  simp [twistedRoot4Case, caseSlot, uuuRepresentativeIndexBySlot,
    ProfileDUUUFiveSupportPositionSymmetry.k16Index]

private theorem twistedRoot4Realization_selectedRootColor
    (realization : TwistedRoot4Realization) :
...
theorem CanonicalStrongData.blockZero_support_iff
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
      ...
      position ∈ (twistedRoot4SupportPair realization).2.1 := by
  ...
  change realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.tail.coloring
    ...
    position ∈ twistedRoot4SmallSupport realization
  rw [TwistedRoot4Realization.smallSupport_eq_foreignZero realization]
  have hmem :=
    (mem_foreignOwnSupport realization.branch.toSemantic 0 position).symm
  rw [twistedRoot4Realization_selectedBlockVertex realization,
    twistedRoot4Realization_selectedRootColor realization] at hmem
  exact hmem
...
theorem CanonicalStrongData.blockOne_support_iff
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
      ...
      position ∈ (twistedRoot4SupportPair realization).1.1 := by
  unfold selectedVertex physicalBlockOneVertex
  rw [totalColoring_tail_tail, data.tail_eq]
  change realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.tail.coloring
      (blockVertex 2 (Fin.last 15).val) (blockVertex 1 position.val) = 3 ↔
    position ∈ twistedRoot4LargePhysicalSupport realization
  rw [TwistedRoot4Realization.largePhysicalSupport_eq realization]
...
private theorem satisfies_support_clause
    ...
    (twistedRoot4FinalValuation realization data).satisfies
      [supportLiteral
        (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
  ...
  · apply satisfies_of_mem_holds
    ...
    · change ¬twistedRoot4FinalValuation realization data
        (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
          selectedVertex target (3 : Fin 4))
      rw [twistedRoot4FinalValuation_ordinaryEdgeAtom realization data
...
theorem twistedRoot4FinalValuation_satisfies_supportUnits
    (realization : TwistedRoot4Realization)
    (data : CanonicalStrongData realization) :
    (twistedRoot4FinalValuation realization data).satisfies_fmla
      (supportUnitClauses (twistedRoot4SupportPair realization)) := by
  constructor
  intro clause hclause
...
theorem semanticAssignment_freeLarge_edge
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
        ...
        (physicalBlockOneVertex
          ((semanticAssignment realization data).1.largePhysical
            (twistedRoot4SupportPair realization) large)) =
  ...
  exact TwistedRoot4Realization.free_large_host_edge realization
    (data.rowPerm free)
...
theorem semanticAssignment_freeSmall_edge
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
        ...
        (physicalBlockZeroVertex
          ((semanticAssignment realization data).1.smallPhysical
            (twistedRoot4SupportPair realization) small)) =
      decodedNormalHostColor
        (k15Template true
          ((semanticAssignment realization data).1.freeLabel free)
  ...
  exact TwistedRoot4Realization.free_small_host_edge realization
    (data.rowPerm free)
...
theorem semanticAssignment_largeSmall_edge
  ...
  rw [
    ...
    semanticAssignmentData_smallLabel realization data small,
    semanticAssignmentData_smallPhysical realization data small]
  exact TwistedRoot4Realization.large_small_host_edge realization
    ((largeLabelEquiv realization).symm (largeLabelOrder large))
    ((smallLabelEquiv realization).symm (smallLabelOrder small))

theorem twistedRoot4FinalValuation_satisfies_selectorFmla
    (realization : TwistedRoot4Realization)
    (data : CanonicalStrongData realization) :
    (twistedRoot4FinalValuation realization data).satisfies_fmla
      (selectorFmla (twistedRoot4SupportPair realization)) := by
  ...
  · have hsingle : clause = selectorAtLeastOneClause
        (twistedRoot4SupportPair realization) := List.mem_singleton.mp hatLeast
    subst clause
    apply satisfies_of_mem_holds
      (l := Sat.Literal.pos (selectorAtom selected))
    · change Sat.Literal.pos (selectorAtom selected) ∈
        (localAssignments (twistedRoot4SupportPair realization)).map
      ...
      · rcases List.mem_append.mp hfirstTwo with hfreeLarge | hfreeSmall
        · obtain ⟨free, _hfree, hlargeClause⟩ :=
          ...
          · change twistedRoot4FinalValuation realization data
            ...
            rw [twistedRoot4FinalValuation_ordinaryEdgeAtom realization data _ _
...
theorem twistedRoot4Realization_satisfies_pairFmla
    (realization : TwistedRoot4Realization)
    (data : CanonicalStrongData realization) :
    (twistedRoot4FinalValuation realization data).satisfies_fmla
      (pairFmla (twistedRoot4SupportPair realization)) := by
  constructor
  intro clause hclause
  ...
  · rcases List.mem_append.mp hleft with hbase | hsupport
    ...
    · exact (twistedRoot4FinalValuation_satisfies_supportUnits
        realization data).prop clause hsupport
  · exact (twistedRoot4FinalValuation_satisfies_selectorFmla
      realization data).prop clause hselector

theorem twistedRoot4Realization_yields_satisfying_pairFmla
    (realization : TwistedRoot4Realization) :
  ...
  exact ⟨twistedRoot4SupportPair realization,
    twistedRoot4FinalValuation realization data,
    twistedRoot4Realization_satisfies_pairFmla realization data⟩
...
theorem noStrongRealization_of_all_pair_lrat
  ...
  obtain ⟨pair, valuation, hsatisfies⟩ :=
    twistedRoot4Realization_yields_satisfying_pairFmla realization
  exact hunsat pair valuation hsatisfies

#print axioms twistedRoot4FreeLabelSet_card
#print axioms twistedRoot4LargeLabelSet_card
#print axioms twistedRoot4SmallLabelSet_card
#print axioms supportUnitClauses_length
#print axioms guardedAssignmentClauses_length
#print axioms selectorFmla_length
#print axioms twistedRoot4Fmla_outside_selectorNamespace
#print axioms semanticAssignmentData_compatible
#print axioms twistedRoot4Realization_satisfies_pairFmla
