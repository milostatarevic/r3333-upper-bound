import PricingG.UUTD9Root1Case
import R4333Lean.ProfileDDegreeTenMatrixCNF

/-! Continuation-authored (locks/duuu), 2026-08-05. Twisted root-12 CNF module,
role-aware clone of ProfileDUUUDegreeNineStrongCaseCNF61 (physical sites 80/83/468 unchanged). -/
...
`UUTD9Root1Case`.  A physical support pair consists of an
...
def twistedRoot1FreeLabelSet : Finset (Fin 15) :=
  colorNeighborhood (k15Template true) 0 1

def twistedRoot1LargeLabelSet : Finset (Fin 15) :=
  colorNeighborhood (k15Template true) 2 1

theorem twistedRoot1FreeLabelSet_card : twistedRoot1FreeLabelSet.card = 5 := by decide

theorem twistedRoot1LargeLabelSet_card : twistedRoot1LargeLabelSet.card = 5 := by decide

theorem twistedRoot1SmallLabelSet_card : twistedRoot1SmallLabelSet.card = 4 := by decide

def twistedRoot1PairOfSupports?
    (large small : Finset (Fin 15)) : Option StrongPair :=
  if hlarge : large ∈ twistedRoot1LargeSupportCandidates then
    if hsmall : small ∈ twistedRoot1CompatibleSmallSupportCandidates then
...
def twistedRoot1LargePhysicalColorMap : ThreeColor → ThreeColor := ![0, 1, 0]

def twistedRoot1LargePhysicalColoring : EdgeColoring (Fin 15) ThreeColor where
  color left right := twistedRoot1LargePhysicalColorMap
    (k15Template false left right)
  color_symm left right := by
    exact congrArg twistedRoot1LargePhysicalColorMap
...
def twistedRoot1FreeNormalColor (left right : FiveVertex) : ThreeColor :=
...
def freeLabelOrder : Fin 5 ≃ ↑twistedRoot1FreeLabelSet :=
  (Finset.orderIsoOfFin twistedRoot1FreeLabelSet twistedRoot1FreeLabelSet_card).toEquiv

def largeLabelOrder : Fin 5 ≃ ↑twistedRoot1LargeLabelSet :=
  (Finset.orderIsoOfFin twistedRoot1LargeLabelSet twistedRoot1LargeLabelSet_card).toEquiv

def smallLabelOrder : Fin 4 ≃ ↑twistedRoot1SmallLabelSet :=
  (Finset.orderIsoOfFin twistedRoot1SmallLabelSet twistedRoot1SmallLabelSet_card).toEquiv
...
def FreePermutationCompatible (freePerm : Equiv.Perm (Fin 5)) : Prop :=
  ∀ left right : FiveVertex, left ≠ right →
    k15Template true
      ...
      twistedRoot1FreeNormalColor left right
...
def LargePermutationCompatible (pair : StrongPair)
  ...
  ∀ left right : Fin 5, left ≠ right →
    k15Template true (largeLabelOrder left).1
      ...
      twistedRoot1LargePhysicalColoring
...
def SmallPermutationCompatible (pair : StrongPair)
  ...
  ∀ left right : Fin 4, left ≠ right →
    k15Template true (smallLabelOrder left).1
      ...
      twistedRoot1SmallPhysicalColoring
...
noncomputable def freeLabelEquiv (realization : TwistedRoot1Realization) :
    FiveVertex ≃ ↑twistedRoot1FreeLabelSet :=
  embeddingEquivRange
    ...
    twistedRoot1FreeLabelSet (by
      simpa [twistedRoot1FreeLabelSet] using realization.freeLabel_image)

noncomputable def largeLabelEquiv (realization : TwistedRoot1Realization) :
    FiveVertex ≃ ↑twistedRoot1LargeLabelSet :=
  embeddingEquivRange
    (⟨largeLabel realization, by
      ...
      apply restLabel_injective_local
      exact realization.normal.vertex.symm.injective h⟩)
    twistedRoot1LargeLabelSet (by
      simpa [twistedRoot1LargeLabelSet] using realization.largeLabel_image)

theorem TwistedRoot1Realization.largeTarget_eq_one
...
theorem TwistedRoot1Realization.largeTargetSize_eq_fifteen
    (realization : TwistedRoot1Realization) :
    blockSize (largeForeignBlock realization.branch.toSemantic.block
      realization.skeleton.firstLarge) = 15 := by
  rw [TwistedRoot1Realization.largeTarget_eq_one realization]
  rfl

noncomputable def largePhysicalEmbedding
    (realization : TwistedRoot1Realization) : Fin 5 ↪ Fin 15 where
  toFun index := Fin.cast
    (TwistedRoot1Realization.largeTargetSize_eq_fifteen realization)
...
noncomputable def twistedRoot1LargePhysicalSupport (realization : TwistedRoot1Realization) :
    Finset (Fin 15) :=
  Finset.univ.image (largePhysicalEmbedding realization)

theorem TwistedRoot1Realization.largePhysicalEmbedding_mem
    (realization : TwistedRoot1Realization) (index : Fin 5) :
    largePhysicalEmbedding realization index ∈
      twistedRoot1LargeSupport realization := by
  ...
  simpa [largePhysicalEmbedding,
    TwistedRoot1Realization.largeTarget_eq_one realization] using hcolor

theorem TwistedRoot1Realization.largePhysicalSupport_eq
    (realization : TwistedRoot1Realization) :
    twistedRoot1LargePhysicalSupport realization =
      twistedRoot1LargeSupport realization := by
  apply Finset.eq_of_subset_of_card_le
  · intro position hposition
    obtain ⟨index, _hindex, rfl⟩ := Finset.mem_image.mp hposition
    exact TwistedRoot1Realization.largePhysicalEmbedding_mem realization index
  · rw [show (twistedRoot1LargeSupport realization).card = 5 from
      (mem_independentSupportCandidates_iff 5 2
        (twistedRoot1LargeSupport realization)).mp
          realization.largeSupport_mem_candidates |>.1]
    simp [twistedRoot1LargePhysicalSupport,
      Finset.card_image_of_injective _
        (largePhysicalEmbedding realization).injective]

theorem TwistedRoot1Realization.largePhysicalSupport_mem_candidates
    (realization : TwistedRoot1Realization) :
    twistedRoot1LargePhysicalSupport realization ∈
      twistedRoot1LargeSupportCandidates := by
  rw [TwistedRoot1Realization.largePhysicalSupport_eq realization]
  exact realization.largeSupport_mem_candidates

noncomputable def twistedRoot1SupportPair
    (realization : TwistedRoot1Realization) : StrongPair :=
  ⟨⟨twistedRoot1LargePhysicalSupport realization,
      TwistedRoot1Realization.largePhysicalSupport_mem_candidates realization⟩,
    ⟨twistedRoot1SmallSupport realization,
      realization.smallSupport_mem_compatibleCandidates⟩⟩

noncomputable def largePhysicalEquiv
    (realization : TwistedRoot1Realization) :
    Fin 5 ≃ ↑(twistedRoot1LargePhysicalSupport realization) :=
  embeddingEquivRange (largePhysicalEmbedding realization)
    (twistedRoot1LargePhysicalSupport realization) rfl

theorem TwistedRoot1Realization.free_free_edge
    (realization : TwistedRoot1Realization) (left right : FiveVertex)
    (hlr : left ≠ right) :
    k15Template true (freeLabel realization left)
...
private theorem decode_blockOne_template
    ...
    decodeNonBlockColor (blockRootColor 2) (canonicalPalette 2 q) =
      twistedRoot1LargePhysicalColorMap q := by
  fin_cases q
...
theorem TwistedRoot1Realization.large_large_edge_physical
    (realization : TwistedRoot1Realization) (left right : Fin 5)
    (hlr : left ≠ right) :
    k15Template true (largeLabel realization left)
  ...
  have hedge :
    ...
    simp [restColoring, pullbackColoring, restEmbedding,
      ...
      ProfileDUUUFiveSupportPositionSymmetry.PositionedFiveSupport.toSemantic,
      twistedRoot1Case, caseSlot, uuuRepresentativeIndexBySlot,
  ...
  have hleftMem : largePhysicalEmbedding realization left ∈
      twistedRoot1LargeSupport realization := by
    exact TwistedRoot1Realization.largePhysicalEmbedding_mem realization left
  have hrightMem : largePhysicalEmbedding realization right ∈
      twistedRoot1LargeSupport realization := by
    exact TwistedRoot1Realization.largePhysicalEmbedding_mem realization right
  have hindependent :=
    (mem_independentSupportCandidates_iff 5 2
      (twistedRoot1LargeSupport realization)).mp
  ...
  simpa [twistedRoot1LargePhysicalColoring] using hedge

/-! The canonical row transport, retaining its permutation explicitly. -/

structure CanonicalStrongData (realization : TwistedRoot1Realization) where
...
theorem exists_canonicalStrongData (realization : TwistedRoot1Realization) :
  ...
  let branch : CanonicalBranch .uut 2 (Fin.last 15) := {
...
noncomputable def semanticFreeEquiv
    ...
    Fin 5 ≃ ↑twistedRoot1FreeLabelSet :=
...
noncomputable def semanticSmallEquiv
    (realization : TwistedRoot1Realization) :
    Fin 4 ≃ ↑(twistedRoot1SmallSupport realization) :=
...
noncomputable def semanticLargePairEquiv
    (realization : TwistedRoot1Realization) :
    Fin 5 ≃ ↑(twistedRoot1SupportPair realization).1.1 := by
  change Fin 5 ≃ ↑(twistedRoot1LargePhysicalSupport realization)
  exact semanticLargeEquiv realization

noncomputable def semanticSmallPairEquiv
    (realization : TwistedRoot1Realization) :
    Fin 4 ≃ ↑(twistedRoot1SupportPair realization).2.1 := by
  change Fin 4 ≃ ↑(twistedRoot1SmallSupport realization)
...
noncomputable def semanticAssignmentData
  ...
  largePerm := (semanticLargePairEquiv realization).trans
    (pairLargeOrder (twistedRoot1SupportPair realization)).symm
  smallPerm := (semanticSmallPairEquiv realization).trans
    (pairSmallOrder (twistedRoot1SupportPair realization)).symm
...
theorem semanticAssignmentData_largePhysical
    ...
    (semanticAssignmentData realization data).largePhysical
        (twistedRoot1SupportPair realization) index =
      largePhysicalEmbedding realization
        ((largeLabelEquiv realization).symm (largeLabelOrder index)) := by
  change (pairLargeOrder (twistedRoot1SupportPair realization)
    (((semanticLargePairEquiv realization).trans
      (pairLargeOrder (twistedRoot1SupportPair realization)).symm) index)).1 = _
  rw [Equiv.trans_apply, Equiv.apply_symm_apply]
...
theorem semanticAssignmentData_smallPhysical
    ...
    (semanticAssignmentData realization data).smallPhysical
      ...
      smallPhysicalEmbedding realization
        ((smallLabelEquiv realization).symm (smallLabelOrder index)) := by
  change (pairSmallOrder (twistedRoot1SupportPair realization)
    (((semanticSmallPairEquiv realization).trans
      (pairSmallOrder (twistedRoot1SupportPair realization)).symm) index)).1 = _
  rw [Equiv.trans_apply, Equiv.apply_symm_apply]
...
theorem decode_canonicalInside_eq_strongFreeNormalColor
  ...
  · simp [twistedRoot1FreeNormalColor, hcycle, decodeRemainingColor,
...
theorem semanticAssignmentData_compatible
    (realization : TwistedRoot1Realization)
    (data : CanonicalStrongData realization) :
    LocalAssignmentCompatible (twistedRoot1SupportPair realization)
  ...
  · intro left right hlr
    ...
    rw [semanticAssignmentData_freeLabel,
      ...
      TwistedRoot1Realization.free_free_edge realization (data.rowPerm left)
  ...
  · intro left right hlr
    ...
    change k15Template true
      ...
      twistedRoot1LargePhysicalColoring
        ((semanticAssignmentData realization data).largePhysical
          (twistedRoot1SupportPair realization) left)
        ((semanticAssignmentData realization data).largePhysical
          (twistedRoot1SupportPair realization) right)
    ...
    exact TwistedRoot1Realization.large_large_edge_physical realization
...
noncomputable def semanticAssignment
    (realization : TwistedRoot1Realization)
    (data : CanonicalStrongData realization) :
    LocalAssignment (twistedRoot1SupportPair realization) :=
  ⟨semanticAssignmentData realization data,
    semanticAssignmentData_compatible realization data⟩

theorem TwistedRoot1Realization.smallSupport_eq_foreignZero
    (realization : TwistedRoot1Realization) :
    twistedRoot1SmallSupport realization =
...
theorem TwistedRoot1Realization.free_large_host_edge
    (realization : TwistedRoot1Realization) (free large : Fin 5) :
    realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.rows
...
theorem TwistedRoot1Realization.free_small_host_edge
    (realization : TwistedRoot1Realization) (free : Fin 5) (small : Fin 4) :
    realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.rows
        free (blockVertex 0
...
theorem TwistedRoot1Realization.large_small_host_edge
    (realization : TwistedRoot1Realization) (large : Fin 5) (small : Fin 4) :
    realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.tail.coloring
...
theorem twistedRoot1BaseFmla_outside_selectorNamespace :
    FormulaOutsideNamespace selectorNamespaceTag
      (baseFmla .uut 2 (Fin.last 15)) := by
  change FormulaOutsideNamespace selectorNamespaceTag
    ((((goodColoringFmla 51 4 ++ templateUnitClauses .uut) ++
    ...
    (formulaOutsideNamespace_append
      (formulaOutsideNamespace_append
        (formulaOutsideNamespace_append
          ...
          (templateUnitClauses_outside_selectorNamespace .uut))
...
theorem twistedRoot1Fmla_outside_selectorNamespace :
    FormulaOutsideNamespace selectorNamespaceTag
      (ProfileDHighOwnFiveSupportC5Formula.fmla
        .uut 2 (Fin.last 15)) := by
  ...
  · exact twistedRoot1BaseFmla_outside_selectorNamespace
  · intro clause hclause literal hliteral
  ...
  (List.finRange 5).flatMap fun free : FiveVertex =>
    (List.finRange 5).map fun large : Fin 5 =>
      guardedEdgeClause assignment
        ...
        (k15Template true (assignment.1.freeLabel free)
  ...
  (List.finRange 5).flatMap fun large : Fin 5 =>
    (List.finRange 4).map fun small : Fin 4 =>
      guardedEdgeClause assignment
        ...
        (physicalBlockZeroVertex
          (assignment.1.smallPhysical pair small))
        (k15Template true (assignment.1.largeLabel large)
          (assignment.1.smallLabel small))
...
def pairFmla (pair : StrongPair) : Sat.Fmla :=
  (fmla .uut 2 (Fin.last 15) ++ supportUnitClauses pair) ++
    selectorFmla pair
...
noncomputable def twistedRoot1FinalValuation
    (realization : TwistedRoot1Realization)
...
theorem twistedRoot1FinalValuation_ordinaryEdgeAtom
    (realization : TwistedRoot1Realization)
    (data : CanonicalStrongData realization)
    (left right : TotalVertex) (hne : left ≠ right) (q : Fin 4) :
    twistedRoot1FinalValuation realization data
      ...
      totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
        left right = q := by
  unfold twistedRoot1FinalValuation
...
private theorem twistedRoot1Realization_selectedBlockVertex
    (realization : TwistedRoot1Realization) :
    blockVertex realization.branch.toSemantic.block
        realization.branch.toSemantic.position.val =
      blockVertex 2 (Fin.last 15).val := by
  change blockVertex
      (uuuRepresentativeIndexBySlot (caseSlot twistedRoot1Case)).1
      (uuuRepresentativeIndexBySlot (caseSlot twistedRoot1Case)).2.val =
    blockVertex 2 (Fin.last 15).val
  simp [twistedRoot1Case, caseSlot, uuuRepresentativeIndexBySlot,
    ProfileDUUUFiveSupportPositionSymmetry.k16Index]

private theorem twistedRoot1Realization_selectedRootColor
    (realization : TwistedRoot1Realization) :
...
theorem CanonicalStrongData.blockZero_support_iff
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
      ...
      position ∈ (twistedRoot1SupportPair realization).2.1 := by
  ...
  change realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.tail.coloring
    ...
    position ∈ twistedRoot1SmallSupport realization
  rw [TwistedRoot1Realization.smallSupport_eq_foreignZero realization]
  have hmem :=
    (mem_foreignOwnSupport realization.branch.toSemantic 0 position).symm
  rw [twistedRoot1Realization_selectedBlockVertex realization,
    twistedRoot1Realization_selectedRootColor realization] at hmem
  exact hmem
...
theorem CanonicalStrongData.blockOne_support_iff
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
      ...
      position ∈ (twistedRoot1SupportPair realization).1.1 := by
  unfold selectedVertex physicalBlockOneVertex
  rw [totalColoring_tail_tail, data.tail_eq]
  change realization.branch.toDegreeFlooredFiveExtension.toFiveExtension.tail.coloring
      (blockVertex 2 (Fin.last 15).val) (blockVertex 1 position.val) = 3 ↔
    position ∈ twistedRoot1LargePhysicalSupport realization
  rw [TwistedRoot1Realization.largePhysicalSupport_eq realization]
...
private theorem satisfies_support_clause
    ...
    (twistedRoot1FinalValuation realization data).satisfies
      [supportLiteral
        (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
  ...
  · apply satisfies_of_mem_holds
    ...
    · change ¬twistedRoot1FinalValuation realization data
        (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
          selectedVertex target (3 : Fin 4))
      rw [twistedRoot1FinalValuation_ordinaryEdgeAtom realization data
...
theorem twistedRoot1FinalValuation_satisfies_supportUnits
    (realization : TwistedRoot1Realization)
    (data : CanonicalStrongData realization) :
    (twistedRoot1FinalValuation realization data).satisfies_fmla
      (supportUnitClauses (twistedRoot1SupportPair realization)) := by
  constructor
  intro clause hclause
...
theorem semanticAssignment_freeLarge_edge
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
        ...
        (physicalBlockOneVertex
          ((semanticAssignment realization data).1.largePhysical
            (twistedRoot1SupportPair realization) large)) =
  ...
  exact TwistedRoot1Realization.free_large_host_edge realization
    (data.rowPerm free)
...
theorem semanticAssignment_freeSmall_edge
    ...
    totalColoring data.branch.toDegreeFlooredFiveExtension.toFiveExtension
        ...
        (physicalBlockZeroVertex
          ((semanticAssignment realization data).1.smallPhysical
            (twistedRoot1SupportPair realization) small)) =
      decodedNormalHostColor
        (k15Template true
          ((semanticAssignment realization data).1.freeLabel free)
  ...
  exact TwistedRoot1Realization.free_small_host_edge realization
    (data.rowPerm free)
...
theorem semanticAssignment_largeSmall_edge
  ...
  rw [
    ...
    semanticAssignmentData_smallLabel realization data small,
    semanticAssignmentData_smallPhysical realization data small]
  exact TwistedRoot1Realization.large_small_host_edge realization
    ((largeLabelEquiv realization).symm (largeLabelOrder large))
    ((smallLabelEquiv realization).symm (smallLabelOrder small))

theorem twistedRoot1FinalValuation_satisfies_selectorFmla
    (realization : TwistedRoot1Realization)
    (data : CanonicalStrongData realization) :
    (twistedRoot1FinalValuation realization data).satisfies_fmla
      (selectorFmla (twistedRoot1SupportPair realization)) := by
  ...
  · have hsingle : clause = selectorAtLeastOneClause
        (twistedRoot1SupportPair realization) := List.mem_singleton.mp hatLeast
    subst clause
    apply satisfies_of_mem_holds
      (l := Sat.Literal.pos (selectorAtom selected))
    · change Sat.Literal.pos (selectorAtom selected) ∈
        (localAssignments (twistedRoot1SupportPair realization)).map
              ...
              (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
                ...
                (physicalBlockOneVertex
                  (selected.1.largePhysical
                    (twistedRoot1SupportPair realization) large))
                (decodedNormalHostColor
                  (k15Template true (selected.1.freeLabel free)
                    (selected.1.largeLabel large)))))
          · exact List.Mem.tail _ (List.Mem.head _)
          · change twistedRoot1FinalValuation realization data
              (OutsideDegreeCapCNF.outsideIncidentEdgeAtom _ _ _)
            rw [twistedRoot1FinalValuation_ordinaryEdgeAtom realization data _ _
            ...
            (OutsideDegreeCapCNF.outsideIncidentEdgeAtom
              ...
              (physicalBlockZeroVertex
                (selected.1.smallPhysical
                  (twistedRoot1SupportPair realization) small))
              (decodedNormalHostColor
                (k15Template true (selected.1.largeLabel large)
                  (selected.1.smallLabel small)))))
        · exact List.Mem.tail _ (List.Mem.head _)
        · change twistedRoot1FinalValuation realization data
            (OutsideDegreeCapCNF.outsideIncidentEdgeAtom _ _ _)
...
theorem twistedRoot1Realization_satisfies_pairFmla
    (realization : TwistedRoot1Realization)
    (data : CanonicalStrongData realization) :
    (twistedRoot1FinalValuation realization data).satisfies_fmla
      (pairFmla (twistedRoot1SupportPair realization)) := by
  constructor
  intro clause hclause
  ...
  · rcases List.mem_append.mp hleft with hbase | hsupport
    ...
    · exact (twistedRoot1FinalValuation_satisfies_supportUnits
        realization data).prop clause hsupport
  · exact (twistedRoot1FinalValuation_satisfies_selectorFmla
      realization data).prop clause hselector

theorem twistedRoot1Realization_yields_satisfying_pairFmla
    (realization : TwistedRoot1Realization) :
  ...
  exact ⟨twistedRoot1SupportPair realization,
    twistedRoot1FinalValuation realization data,
    twistedRoot1Realization_satisfies_pairFmla realization data⟩
...
theorem noStrongRealization_of_all_pair_lrat
  ...
  obtain ⟨pair, valuation, hsatisfies⟩ :=
    twistedRoot1Realization_yields_satisfying_pairFmla realization
  exact hunsat pair valuation hsatisfies

#print axioms twistedRoot1FreeLabelSet_card
#print axioms twistedRoot1LargeLabelSet_card
#print axioms twistedRoot1SmallLabelSet_card
#print axioms supportUnitClauses_length
#print axioms guardedAssignmentClauses_length
#print axioms selectorFmla_length
#print axioms twistedRoot1Fmla_outside_selectorNamespace
#print axioms semanticAssignmentData_compatible
#print axioms twistedRoot1Realization_satisfies_pairFmla
