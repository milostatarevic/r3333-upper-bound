...
Set the four input paths below. `archive_dir` may be the extracted archive
root containing `evidence/`, or the `evidence/` directory itself.
...
cd "$native_proof_dir"
lake build
lake env lean -j1 --run "$common_tools/Emit.lean" "$work_dir/common"
